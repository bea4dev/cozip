    ...
    fn from_args() -> Result<Self, String> {
        ...
        while let Some(arg) = args.next() {
            ...
            match arg.as_str() {
                ...
                "--skip-decompress" => {
                    cfg.skip_decompress = true;
                    continue;
                }
                "--no-skip-decompress" => {
                    cfg.skip_decompress = false;
                    continue;
                }
                "--verify" => {
            ...
            match arg.as_str() {
                ...
                "--verify-bytes" => {
                    cfg.verify_bytes = match value.as_str() {
                        "1" | "true" | "TRUE" | "True" => true,
                        "0" | "false" | "FALSE" | "False" => false,
                        _ => return Err("invalid --verify-bytes (expected 0|1)".to_string()),
                    };
                }
                _ => {
...
fn help_text() -> String {
    let text = r#"usage: cargo run --release -p cozip_pdeflate --example bench_1gb -- [options]
  ...
  --skip-decompress        skip decompression + byte verification (default: on)
  --no-skip-decompress     enable decompression
  --verify                 verify restored bytes when decompression is enabled (default: on)
  --no-verify              disable byte verification
  --verify-bytes <B>       set verification (0/1)
...
fn run_case(
    ...
    for _ in 0..iters {
        ...
        let (decompress_elapsed, decompress_stats) = if skip_decompress {
            (Duration::ZERO, DeflateCpuStreamStats::default())
        } else {
            let mut compressed_reader = Cursor::new(&compressed);
            let mut restored = Vec::with_capacity(input.len());
            let t1 = Instant::now();
            let stats = if let Some(index) = compress_result.index.as_ref() {
                cozip
...
fn main() {
    ...
    println!(
        "gpu_fraction={:.2} gpu_tail_stop_ratio={:.2} token_finalize_segment_size={} gpu_slots={} gpu_batch_chunks={} decomp_gpu_batch_chunks={} gpu_submit_chunks={} stream_pipeline_depth={} stream_batch_chunks={} stream_max_inflight_chunks={} stream_max_inflight_mib={} scheduler={:?} profile_timing={} profile_timing_detail={} profile_timing_deep={} skip_decompress={} verify_bytes={}",
        cfg.gpu_fraction,
